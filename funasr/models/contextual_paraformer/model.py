...
class ContextualParaformer(Paraformer):
    ...
    def cal_decoder_with_predictor(self, encoder_out, encoder_out_lens, sematic_embeds, ys_pad_lens, hw_list=None,
        ...
        # print(sematic_embeds.shape)
        # print(ys_pad_lens.shape)
        # print(hw_embed.shape)
        # import numpy as np
        # enc = encoder_out.detach().numpy()
        # enc_len = encoder_out_lens.detach().numpy().astype(np.int32)
        # pre_acoustic_embeds = sematic_embeds.detach().numpy()
        # pre_token_length = ys_pad_lens.detach().numpy().astype(np.int32)
        # bias_embed = hw_embed.detach().numpy()
        # np.savez("/workspace/tpu-mlir/case/asr/contextual_paraformer/decoder.npz", enc=enc, enc_len=enc_len, pre_acoustic_embeds=pre_acoustic_embeds, pre_token_length=pre_token_length, bias_embed=bias_embed)
        decoder_outs = self.decoder(
    ...
    def inference(self,
        ...
        # print("decoder:", decoder_end-encoder_end)
