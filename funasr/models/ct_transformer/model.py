...
class CTTransformer(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #self.punc_model = EngineOV("./bmodel/punc/punc_bm1688_f32.bmodel", device_id=kwargs['dev_id'])
    ...
    def inference(self,
        ...
        for mini_sentence_i in range(len(mini_sentences)):
            ...
            # y, _ = self.punc_forward(**data)    ## use onnx/bmodel to instead
            # punc_end = time.time()
            # punc_time += (punc_end-punc_start)
            punc_start = time.time()
            inputs = data["text"].type(torch.int32).detach().numpy()
            text_lengths = data['text_lengths'].type(torch.int32).detach().numpy()
            # max_bs = max(max_bs, inputs.shape[0])
            # max_fl = max(max_fl, inputs.shape[1])
            #run bmodel
            output = self.punc_model([inputs, text_lengths])
            # #run onnx
            # import onnxruntime
            # sess = onnxruntime.InferenceSession('/workspace/tpu-mlir/case/asr/gen_punc/punc.onnx')
            # outputs = sess.run(['logits'], {'inputs':inputs, 'text_lengths':text_lengths})
